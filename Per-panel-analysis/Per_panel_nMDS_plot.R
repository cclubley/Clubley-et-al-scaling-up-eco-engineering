...
bio$Func_group <- as.factor(bio$Func_group)
...
bio2 <- dcast(bio, Frame_ID+Configuration+Panel_ID+Panel_type ~ Species, value.var = c("Count_perc"))
colnames(bio2)
...
bio_plot$Panel_type = factor(bio_plot$Panel_type, levels = c("Flat", "Ripple", "cm_25", "cm_5"), ordered=TRUE)

# Separate the species
spc <- bio_plot[, 5:35]

# Create interaction term
int <- interaction(bio_plot$Configuration, bio_plot$Panel_type)
...
ggplot(bio_plot, aes(NMDS1, NMDS2))+
  geom_point(bio_plot, mapping=aes(NMDS1, NMDS2, color=Panel_type, shape=Configuration), size=2, position=position_jitter(.1))+ 
