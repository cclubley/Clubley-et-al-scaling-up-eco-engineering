...
ggplot(data=Mgigas_sum, aes(x=Panel, y=sum))+
  geom_bar(stat="identity", position=position_dodge())+
  scale_fill_viridis(discrete=TRUE, option="magma", begin=0.2, end=0.8)+
  ylab("Total recruited Pacific oysters")+
  xlab("Panel")+
...
set.seed(1234)
mod1 <- glmmTMB(Count ~ Configuration+Panel + (1|Configuration/Frame_ID), ziformula=~1, data=Mgigas, family=poisson)
check_model(mod1)
...
ggplot(data=Aephi_sum, aes(x=Panel, y=sum))+
  geom_bar(stat="identity", position=position_dodge())+
  scale_fill_viridis(discrete=TRUE, option="magma", begin=0.2, end=0.8)+
  ylab("Total recruited Saddle oysters")+
  xlab("Panel")+
...
ggplot(data=Ceum_avg, aes(x=Panel, y=mean))+
  geom_bar(stat="identity", position=position_dodge())+
  scale_fill_viridis(discrete=TRUE, option="magma", begin=0.2, end=0.8)+
  ylab("Number of Corella eumyota")+
  xlab("Panel")+
...
set.seed(1234)
mod1 <- glmmTMB(n2 ~ Configuration+Panel + (1|Configuration/Frame_ID), data=Ceum, family=beta_family())
check_model(mod1)
...
ggplot(data=Wsub_avg, aes(x=Panel, y=mean))+
  geom_bar(stat="identity", position=position_dodge())+
  scale_fill_viridis(discrete=TRUE, option="magma", begin=0.2, end=0.8)+
  ylab("Average cover of Watersipora subatra")+
  xlab("Panel")+
...
set.seed(1234)
mod1 <- glmmTMB(n2 ~ Configuration+Panel + (1|Configuration/Frame_ID), data=Wsub, family=beta_family())
check_model(mod1)
