...
# Sediment depth
Sedd <- read.csv("./Sediment_depth.csv")
...
Sedd$Panel_position <- as.factor(Sedd$Panel_position)
Sedd$Panel_type <- as.factor(Sedd$Panel_type)
# Check again
str(Sedd)
...
# The effect of panel type on cover
Sedc %>% group_by(Panel_type) %>% 
  summarise(mean = mean(Count_perc), se = std.error(Count_perc))
# Panel       Mean     SE
# Flat        65.5   4.07
# Ripple      60.8   3.78
...
ggplot(data=sedc_avg, aes(x=Configuration, y=mean))+
...
sedc_avg <- Sedc %>% group_by(Panel_type) %>% 
  summarise(mean=mean(Count_perc), se=std.error(Count_perc))
# Re-order the panel types
sedc_avg$Panel_type = factor(sedc_avg$Panel_type, levels = c("Flat", "Ripple", "cm_25", "cm_5"), ordered=TRUE)
ggplot(data=sedc_avg, aes(x=Panel_type, y=mean))+
...
ggplot(data=sedd_avg, aes(x=Configuration, y=mean))+
  geom_bar(stat="identity", position=position_dodge())+
  scale_fill_viridis(discrete=TRUE, option="magma", begin=0.2, end=0.8)+
...
# Plot panel on its own
sedd_avg <- Sedd %>% group_by(Panel_type) %>% 
  summarise(mean=mean(Sed_depth), se=std.error(Sed_depth))
# Re-order the panel types
sedd_avg$Panel_type = factor(sedd_avg$Panel_type, levels = c("Flat", "Ripple", "25_cm", "5_cm"), ordered=TRUE)
ggplot(data=sedd_avg, aes(x=Panel_type, y=mean))+
  geom_bar(width=.7, stat="identity", position=position_dodge(.8))+
  scale_fill_viridis(discrete=TRUE, option="magma", begin=0.2, end=0.8)+
  ylab("Average sediment depth (cm)")+
  xlab("Panel")+
